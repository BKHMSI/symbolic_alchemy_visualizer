...
from agents.components.pos_encoding import PositionalEncoding
from agents.components.multihead_attn import MultiHeadAttention
...
class A2C_EPN(nn.Module):
    def __init__(self, config, n_actions):
        ...
        self.memory = Episodic_Memory(1, config["mem-size"], n_feats*2+n_potions+1)

        ############ Multihead Attention ############
        self.pos_enc = PositionalEncoding(self.attn_dim)
        self.attn_proj = nn.Linear(1+n_feats*2+n_potions+self.obs_dim, self.attn_dim)
